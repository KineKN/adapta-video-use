...
- Ad-specific editing references live in:
  ...
  - `skills/ad-corpus-study/references/blind-test-report-2026-07-03.md`
  - `skills/ad-house-funnels/SKILL.md`
  - `skills/ad-copy-structure/SKILL.md`
  - `skills/ad-editorial-decupagem/SKILL.md`
  - `skills/ad-performance-voice/SKILL.md`
  - `skills/ad-semantic-qc/SKILL.md`
  - `skills/ad-corpus-study/SKILL.md`
  - `skills/ad-local-asr-nlp/SKILL.md`
- Post-edit and Premiere handoff references live in:
  - `skills/remove-silence-post/SKILL.md`
  - `skills/premiere-xml-export/SKILL.md`
- Local fallback ASR/NLP environment lives at `.venv_ads`. Prefer it for offline study, WhisperX transcription, and batch-prep utilities.
...
- **`transcribe_whisperx.py <video>`** — local WhisperX fallback in `.venv_ads`; aligned word timings without ElevenLabs.
- **`anuncios_corpus_report.py`** — analyze `Bruto -> Final` pairs and emit markdown/JSON corpus summaries.
- **`anuncios_repetition_candidates.py <txt>`** — detect likely repeated or restarted copy candidates in transcripts.
- **`anuncios_semantic_dedup.py <txt>`** — MPNet-based semantic duplicate detector for near-repeated copy.
- **`anuncios_funnel_classifier.py <txt>`** — classify the ad into a recurring house funnel family before editing.
- **`anuncios_reference_audit.py <Final.txt>`** — audit whether a corpus reference looks strong, review-only, or weak.
- **`anuncios_rewrite_mining.py <Final.txt>`** — subtype rewrite references into `body_reframe`, `full_rebuild`, `hook_swap`, or `cta_rewrite`.
- **`anuncios_reference_policy.py <Final.txt>`** — recommend how much to trust a reference for wording vs structure.
- **`anuncios_reference_anchor_brief.py <Final.txt>`** — extract safe reference anchors compatible with the current reference policy.
- **`anuncios_reference_editor_brief.py <Final.txt>`** — combine funnel, reference audit, rewrite subtype, policy, safe anchors, and hybrid-overlap risk into one ready-to-paste editorial brief for the agent.
- **`anuncios_exception_mining.py --root Anuncios`** — mine low-confidence, unknown, and hybrid-risk cases, labeled with residual priority for calibration work.
- **`anuncios_exception_export.py --root Anuncios --out <json>`** — export the current exception-mining view straight to JSON without shell piping.
- **`anuncios_hybrid_matrix.py --root Anuncios`** — summarize hybrid funnel pairs corpus-wide and classify them as benign, review, or dangerous overlap.
- **`edl_boundary_audit.py <edl.json>`** — flag cut boundaries that are too close to neighboring words, especially tiny leaks from aborted restarts.
- **`remove_silence.py <edl.json>`** — post-decupagem silence tightening. Use only after repetitions/wrong takes are already removed. For audio-threshold tightening, let the helper auto-profile each source from the kept EDL ranges; do not hard-code a universal dB threshold.
- **`export_premiere_xml.py <edl.json>`** — export local FCP 7 XML/XMEML for Premiere Pro import. This is the Premiere handoff path; do not require MCP or a running Premiere instance.
...
5. **Execute.** Produce `edl.json` via the editor sub-agent brief. Drill into `timeline_view` at ambiguous moments. If the user asked for tighter pacing/remove silence, run `remove_silence.py` on the EDL after decupagem and before overlays/final render. Build animations in parallel sub-agents. Apply grade per-segment. Compose via `render.py`.
...
7. **Self-eval (before showing the user).** Run `timeline_view` on the **rendered output** (not the sources) at every cut boundary (±1.5s window). Check each image for:
   ...
   Run `python helpers/edl_boundary_audit.py <edit>/edl.json` before final delivery. Any word boundary closer than ~120ms to a cut edge must be listened to or inspected with `timeline_view`; these are common places where a few milliseconds of an abandoned restart leak through.

   If anything fails: fix → re-render → re-eval. **Cap at 3 self-eval passes** — if issues remain after 3, flag them to the user rather than looping forever. Only present the preview once the self-eval passes.
8. **Iterate + persist.** Natural-language feedback, re-plan, re-render. Never re-transcribe. Final render on confirmation. If the user wants Premiere handoff, export `premiere_export.xml` from the final EDL and give them the XML path for `File > Import` in Premiere. Append to `project.md`.
...
- **Remove silence is not decupagem.** It is a pacing pass after the copy is already correct. Never use it to decide which take or phrase belongs in the ad.
...
- transcript cleanliness is not audio cleanliness
- if a candidate beat sits next to `(cough)`, `(palma)`, crew speech, or obvious setup direction, treat it as suspect until checked against the actual media
- for hooks and CTAs, prefer a slightly shorter clean beat over a fuller beat contaminated by set noise when no clean alternative exists
